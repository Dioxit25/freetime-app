...
> **Коммит:** 7fe8e77 - fix: use prisma db push instead of migrate deploy
> **Статус:** ✅ Timezone исправлен и работает
...
**Статус:** В РАБОТЕ
**Дата:** 2026-03-10
**Описание:** В визуализации календаря неделя начинается с воскресенья (американский формат), а не с понедельника (европейский формат)
**Решение:** Изменить порядок дней в массиве daysOfWeek и логику рендеринга календаря

### 6. Бот не отвечает на команды в группах
**Статус:** ИЗВЕСТНО - ТРЕБУЕТСЯ РЕАЛИЗАЦИЯ
**Дата:** 2026-03-10
**Описание:** Telegram бот не отвечает на команды (/start, /find, /add, /initgroup) в групповых чатах
**Причина:** Бот не реализован в текущем проекте:
- Нет зависимости `telegraf` в package.json
- Нет BOT_TOKEN в переменных окружения
- Нет кода бота или webhook endpoint
**Решение:** Требуется отдельная реализация:
1. Добавить `telegraf` в dependencies
2. Создать мини-сервис бота на порту 3001
3. Настроить webhook с Telegram API
4. Добавить BOT_TOKEN в Vercel environment variables

### 7. Нет кнопки обновления слотов при переключении группы
**Статус:** В РАБОТЕ
**Дата:** 2026-03-10
**Описание:** В поиске свободного времени при переключении группы нет возможности обновить слоты без повторного поиска
**Решение:** Добавить кнопку обновления или автоматическое обновление при смене группы

### 8. Нет выбора участников при поиске слота
**Статус:** В РАБОТЕ
**Дата:** 2026-03-10
**Описание:** При поиске общего свободного времени нет возможности выбрать конкретных участников группы. Поиск всегда идёт по всем участникам.
**Решение:** Добавить UI для выбора/исключения участников, передавать список в API
