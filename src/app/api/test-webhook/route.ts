...
// Test Bot Webhook Handler - uses TEST_BOT_TOKEN for testing changes before production
...
// POST /api/test-webhook - Telegram Test Bot webhook handler
export async function POST(request: NextRequest) {
  // Use TEST_BOT_TOKEN for test bot
  const BOT_TOKEN = process.env.TEST_BOT_TOKEN
  const WEB_APP_URL = process.env.WEB_APP_URL || 'https://freetime-app-jy3k.vercel.app'
  
  if (!BOT_TOKEN) {
    console.error('[TEST BOT] TEST_BOT_TOKEN not configured')
    return NextResponse.json({ error: 'TEST_BOT_TOKEN not configured' }, { status: 500 })
  }
  ...
  try {
    ...
    if (body.my_chat_member) {
      ...
      console.log('[TEST BOT] my_chat_member event:', { chatId: chat.id, oldStatus, newStatus })
      
      // Bot was added to a group
      if (oldStatus === 'left' && (newStatus === 'member' || newStatus === 'administrator')) {
        console.log('[TEST BOT] 🎉 Bot was added to group:', chat.title || chat.id)
        ...
        console.log('[TEST BOT] ✅ Group created/updated:', group.id, group.telegramTitle)
        ...
        await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
          ...
          body: JSON.stringify({
            chat_id: chat.id,
            text: `🧪 *ТЕСТОВЫЙ БОТ*\n\n🎉 Бот добавлен в группу "${chat.title || 'Группа'}"!\n\n⚠️ Это тестовый бот для проверки обновлений.\n\n📱 Команды:\n• /initgroup - инициализировать группу\n• /find - найти общее время\n• /add - добавить занятое время`,
            parse_mode: 'Markdown',
          }),
        })
        
        return NextResponse.json({ ok: true, action: 'bot_added_to_group', bot: 'test' })
      }
      
      return NextResponse.json({ ok: true, action: 'my_chat_member_ignored', bot: 'test' })
    }
    ...
    if (body.message && body.message.text) {
      ...
      console.log('[TEST BOT] Command:', text, '-> Normalized:', command)
      console.log('[TEST BOT] Chat type:', chatType)
      ...
      if (command === '/start') {
        ...
        return NextResponse.json({ ok: true, command: '/start', bot: 'test' })
      }
      
      // Handle /help command
      if (command === '/help') {
        const helpText = `🧪 *ТЕСТОВЫЙ БОТ TimeAgree*
...
💡 Откройте WebApp через меню бота.`
        ...
        return NextResponse.json({ ok: true, command: '/help', bot: 'test' })
      }
      ...
      if (command === '/initgroup') {
        if (chatType === 'private') {
          ...
          await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
            ...
            body: JSON.stringify({
              chat_id: chatId,
              text: `🧪 *ТЕСТОВЫЙ БОТ*\n\n✅ Группа "${body.message.chat.title}" инициализирована!\n\n📱 Теперь участники могут:\n• /find - найти общее время\n• /add - добавить занятость`,
              parse_mode: 'Markdown',
            }),
          })
        }
        
        return NextResponse.json({ ok: true, command: '/initgroup', bot: 'test' })
      }
      ...
      if (command === '/find' || text.startsWith('/find ')) {
        if (chatType === 'private') {
          ...
          return NextResponse.json({ ok: true, command: '/find', error: 'private_chat', bot: 'test' })
        }
        ...
        if (isNaN(daysToLookAhead) || daysToLookAhead < 1 || daysToLookAhead > 30) {
          ...
          return NextResponse.json({ ok: true, command: '/find', error: 'invalid_days', bot: 'test' })
        }
        ...
        await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
          ...
          body: JSON.stringify({
            chat_id: chatId,
            text: `🧪 [TEST BOT] 🔍 Ищем общее свободное время на ${daysToLookAhead} дней...`,
          }),
        })
        ...
        try {
          ...
          if (findResponse.ok) {
            ...
            if (!data.slots || data.slots.length === 0) {
              ...
              let message = `🧪 *ТЕСТОВЫЙ БОТ*\n\n✨ *Найдено ${data.count || data.slots.length} слотов:*
...
`
            ...
            console.error('[TEST BOT] Find time API error:', findResponse.status, errorText)
            await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
          ...
          console.error('[TEST BOT] Find time error:', findError)
          await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        return NextResponse.json({ ok: true, command: '/find', bot: 'test' })
      }
      ...
      if (command === '/add' || text.startsWith('/add ')) {
        if (chatType === 'private') {
          ...
          return NextResponse.json({ ok: true, command: '/add', error: 'private_chat', bot: 'test' })
        }
        ...
        if (args.length < 4) {
          ...
          return NextResponse.json({ ok: true, command: '/add', hint: true, bot: 'test' })
        }
        ...
        if (!dateRegex.test(datePart) || !timeRegex.test(startTime) || !timeRegex.test(endTime)) {
          ...
          return NextResponse.json({ ok: true, command: '/add', error: 'invalid_format', bot: 'test' })
        }
        ...
        if (startDateTime >= endDateTime) {
          ...
          return NextResponse.json({ ok: true, command: '/add', error: 'invalid_time', bot: 'test' })
        }
        ...
        try {
          ...
          console.error('[TEST BOT] Error creating slot:', slotError)
          await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        return NextResponse.json({ ok: true, command: '/add', bot: 'test' })
      }
      
      // Unknown command
      console.log('[TEST BOT] Unknown command:', command)
      return NextResponse.json({ ok: true, action: 'unknown_command', command, bot: 'test' })
    }
    
    // No text message
    return NextResponse.json({ ok: true, action: 'no_text', bot: 'test' })
    ...
    return NextResponse.json(
      { ok: false, error: error.message, bot: 'test' },
      { status: 500 }
    )
  }
}
...
export async function GET(request: NextRequest) {
  return NextResponse.json({ 
    ok: true, 
    message: 'TimeAgree TEST Telegram Webhook',
    bot: 'test',
    botName: '@FreeTimeV2_bot',
