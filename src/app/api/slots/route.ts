...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Verify user, group and membership in a single query for better performance
    const validation = await db.$queryRaw`
      ...
      FROM "User" u
      CROSS JOIN "Group" g
      LEFT JOIN "GroupMember" gm ON gm."userId" = u."id" AND gm."groupId" = g."id"
      WHERE u."id" = ${userId} AND g."id" = ${groupId}
      LIMIT 1
    ` as any[]

    if (!validation || validation.length === 0) {
      console.log('❌ User or group not found in database:', userId, groupId)
      return NextResponse.json(
        { error: 'User or group not found', details: `User ${userId} or group ${groupId} does not exist`, userId, groupId },
        { status: 404 }
      )
    }
